...
pulse.ready(function() {
  ...
  var fireball = new mm.Fireball({
      position: {x: 75, y: 850}
  });

  manLayer.addNode(man);
  manLayer.addNode(fireball);
  ...
  function update(sceneManager, elapsed) {
    ...
    if(arrowLeft) {
      ...
      if(man.state != mm.Megaman.State.Jump) {
        man.state = mm.Megaman.State.Run;
      }
    ...
    if(!arrowLeft && !arrowRight) {
      if(man.state == mm.Megaman.State.Run) {
  ...
  scene.events.bind('keydown', function(e) {
    ...
    if(e.keyCode == 65){
      man.state = mm.Megaman.State.Shoot;
    }
    // Jump with space key
    if(e.keyCode == 32) {
      if(man.state != mm.Megaman.State.Jump) {
        man.state = mm.Megaman.State.Jump;

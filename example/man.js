...
mm.Megaman = pulse.Sprite.extend({
  ...
  init : function(params) {
    ...
    var runAction = new pulse.AnimateAction({
      name : 'run',
      size : {width:55, height:60},
      bounds : {x: 2000, y: 60},
    ...
    //Create animation for shoot
      var shootAction = new pulse.AnimateAction({
          name : 'shoot',
          size : {width:55, height:60},
          bounds : {x: 2000, y: 60},
          frames : [10, 10, 10],
      ...
      // When animation is complete set state back to Idle
      shootAction.events.bind('complete', function(){
          _self.state = mm.Megaman.State.Idle;
      });

      // Add the animation
      this.addAction(shootAction);
  },
  ...
  update : function(elapsed) {
    ...
    if(this._physics.body.GetLinearVelocity().y > 0.01 ||
      ...
      if(this.state != mm.Megaman.State.Intro) {
        this.state = mm.Megaman.State.Jump;
      }
    } else {
      ...
      if(this.state == mm.Megaman.State.Jump) {
        this.state = mm.Megaman.State.Idle;
      }
    }
  ...
  updateState : function(state) {
    ...
    switch(state) {
      ...
      case mm.Megaman.State.Intro:
        ...
        break;
      case mm.Megaman.State.Run:
        this.runAction('run', this._private.oframe);
        break;
      case mm.Megaman.State.Jump:
        this.runAction('jump');
        break;
      case mm.Megaman.State.Smile:
        this.runAction('smile', this._private.oframe);
        break;
      case mm.Megaman.State.Shoot:
        this.runAction('shoot', this._private.oframe);
        break;
    }
  }
...
mm.Megaman.State.Run = 'run';
mm.Megaman.State.Jump = 'jump';
mm.Megaman.State.Smile = 'smile';
mm.Megaman.State.Shoot = 'shoot';
